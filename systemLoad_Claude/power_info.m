...
void printUsage(const char* programName) {
    ...
    printf("  -c           Continuous monitoring (press Ctrl+C to stop)\n");
    printf("  -i <seconds> Interval between measurements in seconds (default: 1, requires -c)\n");
    printf("  -r           Raw output (just the number, no text)\n");
    printf("  -v           Verbose output with timestamp\n");
}
...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        int opt;
        while ((opt = getopt(argc, (char *const *)argv, "hci:rv")) != -1) {
            switch (opt) {
                case 'h':
                    printUsage(argv[0]);
                    return EXIT_SUCCESS;
        ...
        if (!continuousMode) {
            // --- Get All Metrics ---
            float systemPower = getRawSystemPower();
            float adapterPower = getAdapterPower();
            float adapterVoltage = getAdapterVoltage();
            float adapterAmperage = getAdapterAmperage();
            BatteryInfo batteryInfo = getBatteryInfo();
            
            // Get raw battery V/A again for direct display
            ...
            // --- Success Flags ---
            BOOL systemSuccess = systemPower > 0;
            BOOL adapterPowerSuccess = adapterPower > 0; 
            BOOL adapterVoltageSuccess = adapterVoltage >= 0;
            BOOL adapterAmperageSuccess = adapterAmperage >= 0;
            BOOL batteryInfoSuccess = batteryInfo.status != BATTERY_STATUS_FAILED; // Covers calculated power/status
            // Use the direct V/A success flags for their display
            
            // --- Format Strings ---
            const char* batteryStatusStr;
            switch (batteryInfo.status) {
                case BATTERY_STATUS_CHARGING:    batteryStatusStr = "Charging"; break;
                case BATTERY_STATUS_DISCHARGING: batteryStatusStr = "Discharging"; break;
                case BATTERY_STATUS_IDLE:        batteryStatusStr = "Idle"; break;
                default:                         batteryStatusStr = "Failed"; break;
            }

            printf("sys_power=%s adapter_power=%s adapter_voltage=%s adapter_amperage=%s battery_power=%s battery_voltage=%s battery_amperage=%s battery_status=%s\n",
                systemSuccess ? [[NSString stringWithFormat:@"%.2fW", systemPower] UTF8String] : "Failed",
                adapterPowerSuccess ? [[NSString stringWithFormat:@"%.2fW", adapterPower] UTF8String] : "Failed",
                adapterVoltageSuccess ? [[NSString stringWithFormat:@"%.3fV", adapterVoltage] UTF8String] : "Failed", // Changed to %.3fV
                adapterAmperageSuccess ? [[NSString stringWithFormat:@"%.3fA", adapterAmperage] UTF8String] : "Failed",
                batteryInfoSuccess ? [[NSString stringWithFormat:@"%.2fW", batteryInfo.powerWatts] UTF8String] : "N/A",
                batteryVoltageSuccess ? [[NSString stringWithFormat:@"%.3fV", (float)batteryVoltage_mV / 1000.0f] UTF8String] : "Failed",
                batteryAmperageSuccess ? [[NSString stringWithFormat:@"%.3fA", (float)batteryAmperage_mA / 1000.0f] UTF8String] : "Failed",
                batteryStatusStr
            );

            // Exit successfully if system power reading worked (as a basic check)
            return systemSuccess ? EXIT_SUCCESS : EXIT_FAILURE;
        }
        
        // Continuous monitoring mode
        if (!rawOutput) {
            printf("Continuous power monitoring (Ctrl+C to stop)\n");
            if (interval > 1) {
        ...
        while (1) {
            ...
            printf("sys_power=%s adapter_power=%s adapter_voltage=%s adapter_amperage=%s battery_power=%s battery_voltage=%s battery_amperage=%s battery_status=%s\n",
                systemSuccess ? [[NSString stringWithFormat:@"%.2fW", systemPower] UTF8String] : "Failed",
                adapterPowerSuccess ? [[NSString stringWithFormat:@"%.2fW", adapterPower] UTF8String] : "Failed",
                adapterVoltageSuccess ? [[NSString stringWithFormat:@"%.2fV", adapterVoltage] UTF8String] : "Failed", // Changed to %.3fV
                adapterAmperageSuccess ? [[NSString stringWithFormat:@"%.3fA", adapterAmperage] UTF8String] : "Failed",
                batteryInfoSuccess ? [[NSString stringWithFormat:@"%.2fW", batteryInfo.powerWatts] UTF8String] : "N/A",
                batteryVoltageSuccess ? [[NSString stringWithFormat:@"%.3fV", (float)batteryVoltage_mV / 1000.0f] UTF8String] : "Failed",
                batteryAmperageSuccess ? [[NSString stringWithFormat:@"%.3fA", (float)batteryAmperage_mA / 1000.0f] UTF8String] : "Failed",
